...
{
    public partial class SeatManagementWindow : Form
    {
        ...
        private void FrmCheckInSystem_Load(object sender, EventArgs e)
        {
            ...
            {
                cbScheduleList.Items.Add($"{item.Date.ToString("dd/MM/yyyy")}, {item.Time.ToString(@"hh\:mm")}, {item.Route.Airport.IATACode} - {item.Route.Airport1.IATACode}");
            }
            cbScheduleList.SelectedIndex = 0;
        }

        private void ResetAll()
        {
            SeatChart.Visible = false;
        ...
        {
            flpSeats.Controls.Clear();
            var flight = schedules[cbScheduleList.SelectedIndex];
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (rdbSingleCheckin.Checked) singleCheckin = true;
        ...
        private void SearhPairseat()
        {
            updatedTicket = null;
            isChangeSeat = false;
            var controls = flpSeats.Controls;

            if (rdbDualCheckin.Checked)
            {
                label3.Visible = true;
        ...
        {
            SeatChart.Series[0].Points.Clear();
            SeatChart.Series[1].Points.Clear();
            var flight = schedules[cbScheduleList.SelectedIndex];
            ...
            SeatChart.Series[0].Points.AddXY("Left", left);
            SeatChart.Series[0].Points[0].Label = left.ToString();
            SeatChart.Series[1].Points.AddXY("Right", right);
            SeatChart.Series[1].Points[0].Label = right.ToString();

            SeatChart.Visible = true;
        }

        private void RdbDualCheckin_CheckedChanged(object sender, EventArgs e)
        {
            SearhPairseat();
        }

        private void RdbSingleCheckin_CheckedChanged(object sender, EventArgs e)
        {
            SearhPairseat();
        }

        private void RdbSeatChange_CheckedChanged(object sender, EventArgs e)
        {
            SearhPairseat();
        }

        private void cbScheduleList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ResetAll();
        }
    }
